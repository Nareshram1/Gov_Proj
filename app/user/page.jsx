...
export default function UserPage() {
  ...
  const router = useRouter();
    ...
    // 2. useEffect hook to interact with localStorage after component mounts.
    useEffect(() => {
      console.log("Checking localStorage for selfDestructDetails..."); // For debugging
  
      // Try to get the item from localStorage
      const details = localStorage.getItem('selfDestructDetails');
  
      // localStorage.getItem returns null if the key doesn't exist.
      if (details === null) {
        // If 'selfDestructDetails' is NOT found, update the state to false.
        console.log("selfDestructDetails not found. Setting state to false."); // For debugging
        setHasDetails(false);
      } else {
        // Optional: If details ARE found, you might want to set state to true
        // or potentially parse the details if they are stored as JSON.
        console.log("selfDestructDetails found."); // For debugging
        setHasDetails(true);
      }
  ...
  if (hasDetails)
  {
    return(
      ...
      <div className="text-center">
        ...
        <p className="text-gray-500 text-sm mt-4">Please try refreshing the page or contact support if the issue persists.</p>
      </div>
    </div>
    )
  }
